...
window.onload = init;
  function init(){
    ...
    var router = function(){ //function to make a client side router
      ...
      if (showExpression.test(url)){ // if the url matches the regular expression, render the show page.
        view = 'showPage'
      }
      else {
    ...
    function renderView(view) {
      ...
      // no need to getJSON when rendering searchPage because only get data on form submit for that view
    }
    ...
    function handleData(data, view){ //function for handling data
      ...
      else if (data["Title"] && view == 'showPage'){ //if it's the showPage, we queried the omdbapi with an id; its an individual movie
      ...
      else if(view == 'favoritesPage') { // if its the favoritesPage, we queried our own backend api
    ...
    function favoriteMovie(name, oid) {
      ...
      request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8'); // set headers
      request.send(requestSting); // send request
    }
    ...
    //The following is the event listener for the searchPage
